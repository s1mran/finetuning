...
The *reversed* ordering of `cpt_fin_then_sft.py`, run as a controlled experiment:
...
Run this against `cpt_fin_then_sft.py` with the same seed and step counts and
...
Usage
-----
    python sft_fin_then_cpt.py                     # full run, ~15 min on a T4
    python sft_fin_then_cpt.py --smoke             # 30 steps per stage, sanity check
    python sft_fin_then_cpt.py --sft-steps 400 --cpt-steps 400

    # publish to huggingface.co/sidhusarkar (needs `huggingface-cli login` or
    # HF_TOKEN with write scope)
    python sft_fin_then_cpt.py --push-to-hub
    python sft_fin_then_cpt.py --push-to-hub other-user/other-repo --push-adapters
"""
...
OUT = _HERE.parent / "reports" / "sft_fin_then_cpt"   # <repo>/SFT/reports/...
...
Trained with [Unsloth](https://github.com/unslothai/unsloth) — `sft_fin_then_cpt.py`.
"""
...
def main() -> None:
    ...
    # must be identical to the one cpt_fin_then_sft.py builds for the comparison
    ...
    print("Compare against CPT/reports/cpt_fin_then_sft/report.json (same SEED, same "

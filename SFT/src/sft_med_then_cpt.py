...
The *reversed* ordering of `cpt_med_then_sft.py`, run as a controlled experiment:
...
Stage 2 (CPT / continued pre-training)
    Plain next-token prediction on raw clinical prose. Loss on *every* token.
...
the CPT gradient pushes every position toward continuing a clinical guideline, including the
...
Run this against `cpt_med_then_sft.py` with the same seed and step counts and
...
Usage
-----
    python sft_med_then_cpt.py                     # full run, ~15 min on a T4
    python sft_med_then_cpt.py --smoke             # 30 steps per stage, sanity check
    python sft_med_then_cpt.py --sft-steps 400 --cpt-steps 400

    # publish to huggingface.co/sidhusarkar (needs `huggingface-cli login` or
    # HF_TOKEN with write scope)
    python sft_med_then_cpt.py --push-to-hub
    python sft_med_then_cpt.py --push-to-hub other-user/other-repo --push-adapters
"""
...
RUN_NAME = "smollm-135m-med-sft-then-cpt"   # default Hub repo name

_HERE = Path(__file__).resolve().parent
OUT = _HERE.parent / "reports" / "sft_med_then_cpt"   # <repo>/SFT/reports/...
...
|---|---|---|---|
| 1 — SFT | response-only loss on Alpaca-formatted doctor QA | r=16 | attention + MLP |
| 2 — CPT | all-token loss on medical guideline prose (+20% general replay) | r=32 | attention + MLP + `embed_tokens` + `lm_head` |
...
Trained with [Unsloth](https://github.com/unslothai/unsloth) — `sft_med_then_cpt.py`.
"""
...
def main() -> None:
    ...
    # must be identical to the one cpt_med_then_sft.py builds for the comparison
    ...
    report["merge_check"] = verify_merged(
        ...
        report["ppl_after_cpt"]["domain"], "final")
    ...
    print("Compare against CPT/reports/cpt_med_then_sft/report.json (same SEED, same "
